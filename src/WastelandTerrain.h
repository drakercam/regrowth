#ifndef WASTELANDTERRAIN_H
#define WASTELANDTERRAIN_H

#include "Terrain.h"

class WastelandTerrain : public Terrain
{
    public:
        WastelandTerrain(const Transform& t, const ofShader& shader, int w, int h, int res)
        : Terrain(t, shader, w, h, res) 
        {
            noiseScale = 0.02f;
            noiseAmplitude = 20.0f;
            noiseOctaves = 10;
            colorLow = glm::vec3(0.4, 0.3, 0.2);  // dark brown, low areas
            colorMid = glm::vec3(0.5, 0.45, 0.35); // medium brown / gray
            colorHigh = glm::vec3(0.6, 0.55, 0.45); // faded pale brown / gray, high areas
        }
